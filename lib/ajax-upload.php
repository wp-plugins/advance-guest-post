...
global $post;
    ...
    // checking of submit
     jQuery("#wp_store_submit").click(function(e){
			 ...
			 if(  jQuery("#wp_store_content").hasClass("require") && !jQuery("#wp_store_content").val() ){
                 jQuery("#wp_store_content").addClass("req_error");
                 error = 1;
              } 
			   
                  /*
              if(  jQuery("#wp_store_file").hasClass("require") && !jQuery("#wp_store_file").val() ){
                 jQuery("#wp_store_file").addClass("req_error");
                 error = 1;
              }
                  */
...
if(jQuery(".require:checkbox:checked").length > 0){
...
//then loop over the files that were sent and store them using  media_handle_upload();
	...
	if( get_image_type( $_FILES['featured']["tmp_name"] )   ){
				...
				$thumb_id = wp_insert_attachment( $args, $image_url,  1 );
				require_once(ABSPATH . 'wp-admin/includes/image.php');
   ...
   // var_dump( $run );
    ...
    $patched_name = sanitize_file_name( $singe_file ["name"] );
			...
			$uploads = wp_upload_dir();
				...
				$metadata = wp_generate_attachment_metadata( $thumb_id, $image_url );
				wp_update_attachment_metadata( $thumb_id, $metadata );
				$arr[] = array('id'=>$thumb_id, 'url'=>$image_url);
		}
	}
